...
(function (global, factory) {
  ...
  var defaultStrat = function (parentVal, childVal) {
    // 默认策略 没有child 就返回 parent 有child 就返回child
  ...
  // strats: { propsData, el, data, beforeCreate, created, beforeMount, mounted, beforeUpdate, updated,
  //  beforeDestroy, destroyed, activated, deactivated, errorCaptured, serverPrefetch, components,
  //  directives, filters, watch, computed, inject, methods, props, provide }
  ...
  function mergeOptions (
    ...
    var options = {};
    var key;
    // 循环 Vue 中的 options 对象的每个属性
    for (key in parent) {
      mergeField(key);
    }
    // 循环 vm 中的 options 对象的每个属性，并且这个属性在Vue 中没有
    for (key in child) {
   ...
   */
  // 递归遍历一个对象，以执行所有转换的getters，使对象内的每个嵌套属性被收集为到依赖信息
  ...
  function initState (vm) {
    ...
    // data 处理
    // vm.$options.data is mergedInstanceDataFn()
    if (opts.data) {
      initData(vm);
    } else {
  ...
  function createPatchFunction (backend) {
    ...
    function patchVnode (
      ...
      // oldCh老孩子, ch新孩子
      if (isUndef(vnode.text)) { // 新的里面没文本, 就是有子节点
        // 新老孩子都存在
  ...
  function optimize (root, options) {
    ...
    // 标记静态根节点属性 staticRoot (内部全部都是静态节点的父节点就是静态根节点)
